...
def fair_partial_assignment_2_color(df, centers, alpha, beta, color_flag, clustering_method, num_colors, L=0):
    ...
    if clustering_method == "kmeans" or clustering_method == "kmedian":
        ...
        problem, objective = fair_partial_assignment_lp_solver(df, centers, color_flag, alpha, beta, cost_fun_string, L)
        ...
        problem, objective = fair_partial_assignment_lp_solver(df, centers, color_flag, alpha, beta, 'sqeuclidean',L)
        problem.solve()
        ...
        while cost_ub > cost_lb + 0.1:
            ...
            new_problem, new_objective = fair_partial_assignment_lp_solver(df, centers, color_flag, alpha, beta, 'sqeuclidean', L)
...
def fair_partial_assignment_lp_solver(df, centers, color_flag, alpha, beta, cost_fun_string, L):
    ...
    t1 = time.monotonic()
    objects_returned = prepare_to_add_constraints(df, centers, color_flag, beta, alpha, L)
...
def prepare_to_add_constraints(df, centers, color_flag, beta, alpha, L):
    ...
    for var in color_flag:
        ...
        color_constraint, color_rhs = constraint_two_color(num_points, num_centers, var_color_flag, var_beta, var_alpha)
...
def constraint_two_color(num_points, num_centers, prob_vals, beta_val, alpha_val):

    beta_constraints = [[["x_{}_{}".format(j, i) for j in range(num_points)],
                         [(beta_val-prob_vals[j]) for j in range(num_points)]]
                        for i in range(num_centers)]

    alpha_constraints = [[["x_{}_{}".format(j, i) for j in range(num_points)],
                         [(prob_vals[j]-alpha_val) for j in range(num_points)]]
                        for i in range(num_centers)]

    constraints = beta_constraints + alpha_constraints
    number_of_constraints = num_centers * 2
